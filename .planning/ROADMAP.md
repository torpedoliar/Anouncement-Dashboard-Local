...
- **OPD-2 — SSO mode enum divergence (REROUTE/VAULT)**: base enum `PortalSsoMode = FORM/REDIRECT/PROXY/TOKEN`; restricted+multicred spec references REROUTE (`/api/sso/reroute`) and VAULT (SSOCredentialVault). **Repo:** REROUTE + VAULT shipped out-of-band (components SSORerouteSubmit/SSOCredentialVault, route /api/sso/reroute) without enum values. **RESOLVED 2026-08-26:** `PortalSsoMode` is now the official 7-value enum — FORM/REDIRECT/PROXY/TOKEN/REROUTE/VAULT/POST in `prisma/schema.prisma` — completed by migrations `20260722000000_add_portal_and_audit` (base 4 values), `20260729000000_add_sso_mode_reroute`, `20260809000000_add_sso_mode_vault`, `20260821000001_add_sso_mode_post` (POST). End-to-end audit of 13 usage sites found 3 gaps, all fixed: `32c266c` (launch page renders an explicit "SSO Mode X Belum Aktif" status page for REDIRECT/PROXY/TOKEN instead of silently degrading to FORM), `c72332d` (stale REROUTE enum comment aligned with implementation), `db02919` (typo in SSOPostSubmit). REDIRECT/PROXY/TOKEN stay schema-valid but unimplemented — activating them is a separate product decision. Evidence: `.planning/phases/opd2-audit.md` (`npx tsc --noEmit` + scoped eslint green). Closes project decision D8 ⚠️ Revisit.
...
---
*Last updated: 2026-08-26 (OPD-2 closed — SSO enum audit, see phases/opd2-audit.md); created 2026-08-15 after new-project-from-ingest (36 docs, 25 SPECs; 6 conflicting variants preserved as OPD-1..6)*
