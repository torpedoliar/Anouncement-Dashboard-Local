...
to `audit_logs` directly. `logAudit()` is **non-blocking and never throws** (don't wrap it
in try/catch or let it gate the main transaction) and **auto-redacts** sensitive keys
(`password`, `token`, `credentialBlob`, `secret`, …) from `changes`/`metadata` — but don't
rely on that to store secrets on purpose. `ActivityLog` (legacy) is retained + backfilled;
new code uses `AuditLog` exclusively. Admin page: `/admin/audit-trail` (SuperAdmin only)
with export.
...
- **Scheduler** (`lib/scheduler.ts`): no external cron. `runScheduler()` is throttled (60s) and invoked opportunistically from server render (`app/admin/page.tsx`); it (1) auto-publishes (`scheduledAt <= now`), (2) auto-takes-down (`takedownAt <= now`), (3) purges `AuditLog` rows older than `AUDIT_RETENTION_DAYS` (0 = forever). There's also a protected `/api/scheduler` route (Bearer `CRON_SECRET`) for external triggering.
