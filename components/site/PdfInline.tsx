 ...
 * probe-free layered fallback chain (D-05/D-08):
 *   <object application/pdf>  ->  iframe fallback  ->  paragraph + download link
 * and never opens a tab/window automatically. The ONLY tab-opening path is the
 * Fullscreen fallback, and it fires on an explicit click AND only when the
...
export default function PdfInline({ src, filename }: PdfInlineProps) {
    ...
    return (
        <div ref={containerRef} className="pdf-viewer">
            ...
            {/* Layered fallback: object -> iframe -> paragraph + download link.
                CORS / X-Frame / no-PDF-plugin sources degrade to the link, never
                an automatic new tab. */}
            <object
